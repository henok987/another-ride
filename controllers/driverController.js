...
const getDriverById = async (req, res) => {
  try {
    ...
    const sanitizedData = sanitizeForExternalService(driver, requesterRole, serviceType);
...
const listDrivers = async (req, res) => {
  try {
    ...
    const { search, vehicleType, isActive, isVerified, serviceType = 'ride' } = req.query;
    ...
    // Determine requester role from authentication
    const requesterRole = req.user?.role || 'public';
    const sanitizedDrivers = sanitizeBatchUserData(drivers, requesterRole, serviceType);
