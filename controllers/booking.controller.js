...
exports.create = async (req, res) => {
  try {
    ...
    if (!passengerId)
      return res
        .status(400)
        .json({ message: 'Invalid passenger ID: user not authenticated' });
    ...
    // Save booking with only IDs and trip info
    ...
    // Fetch passenger details from User Service
    const passengerResult = await getPassengerDetails(passengerId);
    ...
    return res
      .status(500)
      .json({ message: `Failed to create booking: ${e.message}` });
  }
};
...
exports.list = async (req, res) => {
  try {
    ...
    if (driverIds.length) {
      try {
        const infos = await getDriversByIds(driverIds);
    ...
    return res
      .status(500)
      .json({ message: `Failed to retrieve bookings: ${e.message}` });
  }
};
...
exports.get = async (req, res) => {
  try {
    ...
    const passengerResult = await getPassengerDetails(item.passengerId);
    const driverResult = item.driverId
    ...
    return res
      .status(500)
      .json({ message: `Failed to retrieve booking: ${e.message}` });
  }
};
...
exports.update = async (req, res) => {
  try {
    ...
    return res
      .status(500)
      .json({ message: `Failed to update booking: ${e.message}` });
  }
};
...
exports.remove = async (req, res) => {
  try {
    const r = await Booking.findOneAndDelete({
      _id: req.params.id,
      passengerId: String(req.user?.id),
    });
    if (!r)
      return res.status(404).json({
    ...
    return res
      .status(500)
      .json({ message: `Failed to delete booking: ${e.message}` });
  }
};

// --- Booking Lifecycle (accept, ongoing, complete, cancel) ---
exports.lifecycle = async (req, res) => {
  try {
    ...
    if (
      !['requested', 'accepted', 'ongoing', 'completed', 'canceled'].includes(
        status
      )
    )
    ...
    if (booking.status === 'completed') {
      return res
        .status(400)
        .json({ message: 'Cannot change status of completed bookings' });
    }

    // Example driver acceptance logic...
    ...
    return res.status(500).json({
      message: `Failed to update booking lifecycle: ${e.message}`,
    });
  }
};
...
exports.assign = async (req, res) => {
  try {
    ...
    if (booking.status !== 'requested') {
      return res
        .status(400)
        .json({ message: `Cannot assign booking with status '${booking.status}'` });
    }
    ...
    return res
      .status(500)
      .json({ message: `Failed to assign booking: ${e.message}` });
  }
};
  ...
  try {
    ...
    if (!pickup || !dropoff)
      return res.status(400).json({
        message: 'Pickup and dropoff locations are required for fare estimation',
      });
    const est = await estimateFare({ vehicleType, pickup, dropoff });
    return res.json(est);
  } catch (e) {
    return res
      .status(500)
      .json({ message: `Failed to estimate fare: ${e.message}` });
  }
};
...
exports.ratePassenger = async (req, res) => {
  try {
    ...
    if (booking.driverId !== driverId) {
      return res
        .status(403)
        .json({ message: 'Only the assigned driver can rate the passenger' });
    }

    if (booking.status !== 'completed') {
      return res.status(400).json({ message: 'Can only rate after completion' });
    }
    ...
    return res
      .status(500)
      .json({ message: `Failed to rate passenger: ${e.message}` });
  }
};
...
exports.rateDriver = async (req, res) => {
  try {
    ...
    if (String(booking.passengerId) !== String(passengerId)) {
      return res
        .status(403)
        .json({ message: 'Only the passenger can rate the driver' });
    }
    ...
    return res
      .status(500)
      .json({ message: `Failed to rate driver: ${e.message}` });
  }
};
