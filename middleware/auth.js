...
// Enhanced authentication middleware for external service access
const authenticateExternalService = async (req, res, next) => {
  try {
    ...
    if (serviceToken && serviceName) {
      const validServices = ['ride-service', 'payment-service', 'notification-service'];
    ...
    if (!authHeader || !authHeader.startsWith('Bearer ')) {
      return res.status(401).json({
        error: 'Unauthorized',
        message: 'Access token or service token required'
      });
    }
    ...
    return res.status(500).json({
      ...
      message: 'Failed to authenticate user or service'
    });
  }
};

// Original authentication middleware (for backward compatibility)
